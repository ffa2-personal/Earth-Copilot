...
"""
    ...
    # ------------------------------------------------------------------
    # Profiles added to unblock built-in "Get Started" queries (audit
    # surfaced 7 collections referenced by quickstart_cache.py but
    # missing from this registry). Each profile follows the same shape
    # as neighboring entries so downstream consumers (semantic_translator,
    # build_stac_query) treat them consistently.
    # ------------------------------------------------------------------

    "noaa-c-cap": {
            "name": "C-CAP Regional Land Cover and Change",
            "category": "landcover",
            "resolution": "30m",
            "status": "good",
            "visualization": {
                    ...
                    "assets": {"data": "land cover class"},
            },
            "temporal": {
                    "type": "composite",
                    "epochs": ["1996", "2001", "2006", "2010", "2016"],
                    "notes": "5-year composite epochs; not continuous."
            },
            "platform": "NOAA Office for Coastal Management",
            "usage": "Coastal U.S. land cover and change analysis between published epochs.",
            "query_rules": {
                    "type": "composite_landcover",
                    "description": "NOAA C-CAP coastal land cover -- 5-year composite epochs",
                    "required_params": ["bbox"],
                    "supported_params": ["bbox", "sortby", "limit", "collections", "datetime"],
                    "ignored_params": ["query.eo:cloud_cover", "query.cloud_cover"],
                    "capabilities": {
                            "temporal_filtering": "use_sortby",
                            "cloud_filtering": False,
                            "spatial_filtering": True,
                    ...
                    "agent_guidance": "Composite epochs only (1996/2001/2006/2010/2016). Prefer sortby:datetime desc to surface the newest epoch; only filter by datetime when the user names a specific epoch year."
            }
    },
    ...
    "mtbs": {
            ...
            "visualization": {
                    "type": "categorical",
                    "renderer": "classification",
                    "assets": {"burn_severity": "burn severity class"},
            },
            "temporal": {
                    "type": "annual_composite",
                    "notes": "Annual U.S. burn severity per fire event."
            },
            "platform": "USGS/USFS Monitoring Trends in Burn Severity",
            "usage": "Per-fire burn severity classification across the U.S.",
            "query_rules": {
                    "type": "annual_composite",
                    "description": "MTBS annual burn severity",
                    "required_params": ["bbox"],
                    "supported_params": ["bbox", "datetime", "sortby", "limit", "collections"],
                    "ignored_params": ["query.eo:cloud_cover", "query.cloud_cover"],
                    "capabilities": {
                            "temporal_filtering": True,
                            "cloud_filtering": False,
                    ...
                    "agent_guidance": "Use datetime year filter when user names a fire year; otherwise sortby:datetime desc for the latest available."
            }
    },
    ...
    "jrc-gsw": {
            ...
            "visualization": {
                    "type": "categorical",
                    "renderer": "classification",
                    "assets": {"occurrence": "water occurrence"},
            },
            "temporal": {
                    "static": True,
            ...
            "query_rules": {
                    ...
                    "description": "JRC Global Surface Water -- multi-decadal composite",
                    "required_params": ["bbox"],
                    ...
                    "agent_guidance": "Static composite; do NOT add datetime or sortby:datetime. Only spatial queries."
            }
    },

    "noaa-cdr-sea-surface-temperature-whoi": {
            "name": "NOAA CDR Sea Surface Temperature (WHOI)",
            "category": "ocean",
            "resolution": "0.25 deg",
            "status": "good",
            "visualization": {
                    "type": "continuous",
                    "renderer": "colormap",
                    "assets": {"sea_surface_temperature": "SST (K)"},
                    "colormap": "thermal"
            },
            "temporal": {
                    "type": "dynamic_timeseries",
                    "cadence": "3-hourly",
                    "notes": "Continuous SST analysis; supports datetime filtering."
            },
            "platform": "NOAA Climate Data Record / WHOI",
            "usage": "Sea surface temperature analysis and anomaly studies.",
            "query_rules": {
                    "type": "dynamic_timeseries",
                    "description": "NOAA SST CDR (WHOI) 3-hourly",
                    "required_params": ["bbox"],
                    "supported_params": ["bbox", "datetime", "sortby", "limit", "collections"],
                    "ignored_params": ["query.eo:cloud_cover", "query.cloud_cover"],
                    "capabilities": {
                    ...
                    "agent_guidance": "Use datetime windows for specific events; otherwise sortby:datetime desc for the most recent retrieval."
            }
    },
    ...
    "usda-cdl": {
            ...
            "visualization": {
                    "type": "categorical",
                    "renderer": "classification",
                    "assets": {"cropland": "crop class"},
            },
            "temporal": {
                    "type": "annual_composite",
                    "cadence": "annual",
                    "notes": "One CONUS classification per crop year."
            },
            "platform": "USDA NASS",
            "usage": "Crop type mapping and year-over-year cropland change for the U.S.",
            "query_rules": {
                    "type": "annual_composite",
                    "description": "USDA Cropland Data Layer (CDL)",
                    "required_params": ["bbox"],
                    "supported_params": ["bbox", "datetime", "sortby", "limit", "collections"],
                    "ignored_params": ["query.eo:cloud_cover", "query.cloud_cover"],
                    "capabilities": {
                    ...
                    "agent_guidance": "Filter by year (datetime) when user names a crop year; otherwise sortby:datetime desc."
            }
    },

    "chloris-biomass": {
            "name": "Chloris Aboveground Biomass",
            "category": "vegetation",
            "resolution": "4.6km",
            "status": "good",
            "visualization": {
                    "type": "continuous",
                    "renderer": "colormap",
                    "assets": {"biomass": "aboveground biomass (Mg/ha)"},
                    "colormap": "viridis"
            },
            "temporal": {
                    "type": "annual_composite",
                    "cadence": "annual",
                    "notes": "Annual aboveground biomass estimates (2003-present)."
            },
            "platform": "Chloris Geospatial",
            "usage": "Aboveground biomass and carbon stock change tracking.",
            "query_rules": {
                    "type": "annual_composite",
                    "description": "Chloris annual biomass",
                    "required_params": ["bbox"],
                    "supported_params": ["bbox", "datetime", "sortby", "limit", "collections"],
                    "ignored_params": ["query.eo:cloud_cover", "query.cloud_cover"],
                    "capabilities": {
                    ...
                    "agent_guidance": "Annual product; sortby:datetime desc gives the most recent year."
            }
    },
    ...
    "3dep-lidar-hag": {
            ...
            "visualization": {
                    "type": "continuous",
                    "renderer": "colormap",
                    "assets": {"data": "height above ground (m)"},
                    "colormap": "viridis"
            },
            "temporal": {
                    "type": "rolling_acquisition",
                    "notes": "Tiles acquired across years; treat as quasi-static -- rely on bbox, not datetime."
            },
            "platform": "USGS 3D Elevation Program",
            "usage": "High-resolution height-above-ground analysis (e.g., building/canopy heights) over U.S. coverage tiles.",
            "query_rules": {
                    "type": "rolling_static",
                    "description": "USGS 3DEP LiDAR HAG -- rolling tile acquisition",
                    "required_params": ["bbox"],
                    ...
                    "agent_guidance": "Avoid datetime filters -- not all areas have a recent acquisition. Spatial bbox query only."
            }
    },
